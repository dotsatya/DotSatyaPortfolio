...
export default function TConnection() {
  ...
  const handleEnter = async () => {
    ...
    if (step === "idea") {
      ...
      try {
        await emailjs.send(serviceId, templateId, finalData, publicKey);
  ...
  return (
    <section id="feedback" className=" max-w-5xl mx-auto mt-6">
      <CommandPrompt
        command="./connection.sh --let's connect"
        delay={200}
        path="~/feedback"
      />
      <div className="  rounded-lg p-4 font-mono text-sm bg-white dark:bg-[#0d1117] shadow-md border border-slate-300 dark:border-[#30363d]  text-slate-800 dark:text-[#c9d1d9]">
             <motion.div
        initial={{
          opacity: 0,
          x: -60,
